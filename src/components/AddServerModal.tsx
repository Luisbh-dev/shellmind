...
import { X, Server, Monitor, Loader2 } from 'lucide-react';
...
export default function AddServerModal({ isOpen, onClose, onAdd, initialData }: AddServerModalProps) {
  ...
  const [type, setType] = useState<'linux' | 'windows'>('linux');
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
                ...
                >
                    ...
                    <span className="text-xs font-medium">RDP / Windows</span>
                </button>
            </div>
            ...
            <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                    ...
                    <div>
                        <label className="block text-[10px] uppercase text-zinc-500 font-bold mb-1">{type === 'windows' ? 'RDP Port' : 'SSH Port'}</label>
                        <input 
                            type="number" 
                            value={formData.port}
                            onChange={e => setFormData({...formData, port: e.target.value})}
                            className="w-full bg-black border border-zinc-800 text-zinc-200 text-sm px-3 py-2 rounded focus:border-blue-600 focus:outline-none"
                            placeholder={type === 'linux' ? "22" : "3389"}
                        />
                    </div>
                </div>
