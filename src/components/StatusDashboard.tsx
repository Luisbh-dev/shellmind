  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4 shadow-[0_12px_30px_rgba(0,0,0,0.28)]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-xs uppercase tracking-[0.24em] text-zinc-400">{title}</div>
...
export default function StatusDashboard({ server, isVisible }: StatusDashboardProps) {
  ...
  const fetchStatus = async () => {
    ...
    try {
      setError(null);
      const res = await fetch(`${API_BASE}/api/servers/${server.id}/status`);
    ...
    <div className="absolute inset-0 overflow-y-auto bg-[radial-gradient(circle_at_top_left,_rgba(20,184,166,0.08),_transparent_26%),radial-gradient(circle_at_top_right,_rgba(59,130,246,0.08),_transparent_22%),#0a0a0a]">
      <div className="mx-auto max-w-6xl p-6 md:p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="text-xs uppercase tracking-[0.28em] text-teal-400/80">Status</div>
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/80 px-3 py-2 text-xs text-zinc-400">
              <div className="flex items-center gap-2">
                {isLoading && <RefreshCw className="h-4 w-4 animate-spin text-teal-300" />}
                <span>
          ...
          <>
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-[1.4fr_1fr]">
              <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-5 shadow-[0_12px_30px_rgba(0,0,0,0.28)]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.24em] text-zinc-400">Host</div>
                ...
                <div className="mt-5 grid gap-3 md:grid-cols-3">
                  <div className="rounded-xl border border-zinc-800 bg-black/30 p-3">
                    <div className="text-xs uppercase tracking-[0.2em] text-zinc-400">Free memory</div>
                    <div className="mt-1 text-lg font-medium text-zinc-100">{formatMemoryMB(snapshot.memory.freeMB)}</div>
                  </div>
                  <div className="rounded-xl border border-zinc-800 bg-black/30 p-3">
                    <div className="text-xs uppercase tracking-[0.2em] text-zinc-400">Free storage</div>
                    <div className="mt-1 text-lg font-medium text-zinc-100">{formatDiskGB(snapshot.storage.freeGB)}</div>
                  </div>
                  <div className="rounded-xl border border-zinc-800 bg-black/30 p-3">
                    <div className="text-xs uppercase tracking-[0.2em] text-zinc-400">Platform</div>
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-5 shadow-[0_12px_30px_rgba(0,0,0,0.28)]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.24em] text-zinc-400">Load Signals</div>
                  ...
                  <div className="mt-5 grid grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={entry.label} className="rounded-xl border border-zinc-800 bg-black/30 p-3">
                        <div className="text-xs uppercase tracking-[0.2em] text-zinc-400">{entry.label}</div>
            ...
            <div className="mt-4 grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
              <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-5 shadow-[0_12px_30px_rgba(0,0,0,0.28)]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.24em] text-zinc-400">Top Processes</div>
                ...
                <div className="mt-5 overflow-hidden rounded-xl border border-zinc-800">
                  <div className="grid grid-cols-[minmax(0,1fr)_90px_110px] bg-zinc-900/70 px-4 py-2 text-xs uppercase tracking-[0.22em] text-zinc-400">
                    <span>Process</span>
                    <span>CPU</span>
                    <span>Memory</span>
                  </div>
                    ...
                    {processRows.length ? processRows.map((process) => (
                      <div key={`${process.pid}-${process.name}`} className="grid grid-cols-[minmax(0,1fr)_90px_110px] items-center px-4 py-3 text-sm">
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-zinc-400">PID {process.pid}</div>
                        </div>
                      ...
                      <div className="px-4 py-6 text-sm text-zinc-400">No process data returned.</div>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-5 shadow-[0_12px_30px_rgba(0,0,0,0.28)]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.24em] text-zinc-400">Volumes</div>
                    ...
                    <div key={`${disk.name}-${disk.mount}`} className="rounded-xl border border-zinc-800 bg-black/30 p-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="truncate text-xs text-zinc-400">{disk.mount}</div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm font-medium text-zinc-200">{disk.usagePercent.toFixed(1)}%</div>
                          <div className="text-xs text-zinc-400">{formatDiskGB(disk.usedGB)} / {formatDiskGB(disk.totalGB)}</div>
                        </div>
                      </div>
                      <div className="mt-3">
                        <MetricBar
